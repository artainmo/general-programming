...
You can even generate the code of a whole project simply by explaining in markdown files what needs to be done and in skill files how so (we call this 'spec-driven-development'). You can place different agents in different directories so that they each have their own instruction files (markdown and skills files). For example one directory will be responsible for testing, another one for the frontend of the code, and so forth... 
